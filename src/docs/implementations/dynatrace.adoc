...
[source,java]
----
// In this case, endpoint and token are read from environment variables, but they could also be passed via command line parameters, for example.
String readEndpoint = System.getenv("DT_METRICS_INGEST_URL");
// Use local OneAgent endpoint if reading from the environment fails.
final String endpoint = readEndpoint == null ? "http://localhost:14499/metrics/ingest" : readEndpoint;
String readApiToken = System.getenv("DT_METRICS_INGEST_API_TOKEN");
final String apiToken = readApiToken == null ? "" : readApiToken;
...
management.metrics.export.dynatrace:
    ...
    # Using just device id (without v1) is deprecated, but will work to maintain backwards compatibility.
    v1:
        device-id: sample
