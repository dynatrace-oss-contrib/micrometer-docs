...
These properties can also be set via Spring Boot properties.
...
Otherwise, the version defaults to `v2` automatically, and does not have to be set explicitly.
...
Therefore, if the `deviceId` property is set, all data will be exported to the v1 endpoint.
