...
Since Spring Boot version 3.0.0, the prefix used in Spring Boot changed to `management.dynatrace.metrics.export`.
Please replace `management.metrics.export.dynatrace` with `management.dynatrace.metrics.export` when using Spring Boot 3.0.0 and above, and refer to the https://docs.spring.io/spring-boot/docs/current/reference/html/actuator.html#actuator.metrics.export.dynatrace[Spring Boot documentation].
...
It is also possible to reference environment variables using the Spring property placeholders (e.g.: `management.metrics.export.dynatrace.uri: ${DT_METRICS_INGEST_URL}` / `management.dynatrace.metrics.export.uri` for Spring Boot 3.0.0 and up).
...
# When using Spring Boot 3.0.0 or above, use the line below instead of the line above.
