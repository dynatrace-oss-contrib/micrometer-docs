...
In the minimal configuration <<bookmark-auto-configuration, shown above>> (no URI or API token), the v2 exporter will attempt to retrieve the endpoint provided by the Dynatrace Kubernetes operator.
