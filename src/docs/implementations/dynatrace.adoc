...
management.metrics.export.dynatrace:
    ...
    # should not be hardcoded but rather read from a secure source.
...
For more information about the metadata picked up by the Dynatrace metadata enrichment, see https://www.dynatrace.com/support/help/how-to-use-dynatrace/metrics/metric-ingestion/ingestion-methods/enrich-metrics/[the Dynatrace documentation].
