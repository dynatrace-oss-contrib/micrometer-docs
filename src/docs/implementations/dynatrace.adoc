...
For setting up new integrations with Dynatrace, it is recommended to use the https://www.dynatrace.com/support/help/dynatrace-api/environment-api/metric-v2/[latest version] of the https://www.dynatrace.com/support/help/dynatrace-api/[API] (`api-version: v2`).

Use the following code in your project to export Micrometer metrics to the specified Dynatrace endpoint. 
https://www.dynatrace.com/support/help/dynatrace-api/basics/dynatrace-api-authentication/[The Dynatrace API token documentation] contains more information on how to create an API token.
...
DynatraceConfig dynatraceConfig = new DynatraceConfig() {
    ...
    @Override
    public String uri() {
        // Read the URI from the environment variables or a config file
        // Should contain the path, e. g.: https://your-environment-id.live.dynatrace.com/api/v2/metrics/ingest
    ...
    public String apiToken() {
        ...
        // We do *not* recommend hardcoding token or URI here.
        return MY_TOKEN;
    }
...
If no local OneAgent is running on the machine or the metrics should be sent to a specific endpoint, the Dynatrace v2 exporter can be configured with a Dynatrace environment URL and an https://www.dynatrace.com/support/help/dynatrace-api/basics/dynatrace-api-authentication/[API token].
...
Please also ensure that the endpoint URL is complete, i. e. it contains the path `/api/v2/metrics/ingest` on SaaS and managed deployments, or `/metrics/ingest` for OneAgent endpoints.
...
management.metrics.export.dynatrace:
    ...
    # We do *not* recommend putting the token or URI into the .yml file in clear text.
    # We suggest using environment variables, e. g. api-token: ${ENV_VAR_DT_INGEST_TOKEN}
    api-token: YOUR_TOKEN

    # Sets a prefix that is prepended to each exported metric key.
    metric-key-prefix: your.key.prefix

    # If set to true and a local OneAgent is running, exports OneAgent metadata to the specified Dynatrace endpoint (default: false)
...
DynatraceConfig dynatraceConfig = new DynatraceConfig() {
    @Override
    public String uri() {
        // Should *not* contain the path, e. g.: https://your-environment-id.live.dynatrace.com
        return MY_DYNATRACE_URI;
    }
