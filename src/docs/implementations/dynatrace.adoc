...
management.metrics.export.dynatrace:
    ...
    v2:
        ...
        # (from 1.9.0) Wheter or not to use the Dynatrace specific summary instruments. (default: true)
        # This should only be used if problems with existing instrumentation were discovered after upgrading to 1.9.0
        # Set to false, this will restore the previous (1.8.+) behavior for Timers and DistributionSummaries.
...
DynatraceConfig dynatraceConfig = new DynatraceConfig() {
    ...
    // Only available in Micrometer 1.9.0 and later!
    @Override
