...
For setting up new integrations with Dynatrace, it is recommended to use the latest version of the API (`api-version: v2`).
...
The metrics ingest (`metrics.ingest`) permission is required on the token in order to ingest metrics.
More info on how to use the v2 API is provided <<bookmark-apiv2, below>>.
...
management.metrics.export.dynatrace:
    ...
    api-token: YOUR_TOKEN
    uri: https://{your-environment-id}.live.dynatrace.com // or "https://{your-domain}/e/{your-environment-id}/" on Managed
